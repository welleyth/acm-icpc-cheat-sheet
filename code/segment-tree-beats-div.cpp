/* correct way of handling division of arbitrary 'a' by positive 'b' */
int64_t division(const int64_t a, const int64_t b) {
    if (a >= 0) {
        return a / b;
    } else {
        return -((-a + b - 1) / b);
...
class DivideSegmentTree {
    ...
    /* /= val on segment [ql, qr) */
    void updateDivideEq(const int v, const int l, const int r, const int ql, const int qr, const int val) {
        if (qr <= l || r <= ql) {
            return;
        }
        if (ql <= l && r <= qr && tree[v].max - tree[v].min <= 1) {
            const int64_t maxDiff = division(tree[v].max, val) - tree[v].max;
            const int64_t minDiff = division(tree[v].min, val) - tree[v].min;
            if (maxDiff == minDiff) {
        ...
        pushToChildren(v, l, r);
        const int mid = (r + l) / 2;
        updateDivideEq(leftChild(v), l, mid, ql, qr, val);
        updateDivideEq(rightChild(v), mid, r, ql, qr, val);
        updateFromChildren(v);
    }
...
public:
    ...
    DivideSegmentTree(const std::vector<int>& inputArray) {
        n = inputArray.size();
        tree.resize(4 * n);
    ...
    /* /= val on segment [ql, qr) */
    void updateDivideEq(const int ql, const int qr, const int val) {
        if (val != 1) {
